...
export function useApiPolling(userId: string, features: BehaviorFeatures) {
  ...
  const [status, setStatus] = useState<"active" | "warning" | "locked">("active")
  ...
  const [trendData, setTrendData] = useState<Array<{ timestamp: string; score: number }>>([])
  const [activityLog, setActivityLog] = useState<Activity[]>([])
  const [lastStatus, setLastStatus] = useState<"active" | "warning" | "locked">("active")
  ...
  const submitBehavior = useCallback(
    async (behaviorFeatures: BehaviorFeatures) => {
      try {
        console.log("[v0] Submitting behavior data:", { userId, features: behaviorFeatures })
        ...
        if (data.status === "locked") {
          ...
          console.log("[v0] Session locked until:", new Date(fiveMinutesFromNow).toLocaleTimeString())
          ...
          autounlockRef.current = setTimeout(
            () => {
              ...
              addActivity("Auto-unlocked", 90, "active", "Session automatically resumed")
        ...
        if (data.status !== lastStatus) {
          const statusText = data.status === "active" ? "Normal" : data.status === "warning" ? "Warning" : "Locked"
        ...
        const mockStatus = mockScore > 75 ? "active" : mockScore >= 65 ? "warning" : "locked"
  ...
  const reAuthenticate = useCallback(
    async (password: string) => {
      try {
        ...
        setLockoutUntil(null)
        if (autounlockRef.current) clearTimeout(autounlockRef.current)
        addActivity("Re-authenticated", 90, "active", "User successfully re-authenticated")
        setLastStatus("active")
  ...
  const addActivity = useCallback((description: string, score: number, stat: string, detail: string) => {
